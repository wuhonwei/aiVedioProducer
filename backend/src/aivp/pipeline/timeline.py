...
def build_timeline(chunks_meta: list[dict], extracts: dict[tuple[str, str], dict]) -> list[dict]:
    ...
    """Flatten per-chunk events into a timeline, dropping empty/exact-duplicate summaries."""
...
def write_timeline_pages(
    ...
    *,
    ...
    # Clear previous pages so resume/force does not leave stale tails.
...
def run_timeline(
    ...
    *,
    ...
    if enriched_json is not None and enriched_json.exists():
        ...
        # Deduplicate legacy enriched files that were built before summary de-dupe.
